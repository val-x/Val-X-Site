...
const Stats = ({
  ...
  const variants = {
    default: {
      container:
        "bg-gradient-to-b from-slate-900/80 to-slate-800/80 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6",
      number:
    ...
    gradient: {
      container:
        "bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6 hover:border-slate-600/50 transition-all duration-300",
      number: "text-white",
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        {stats.map((stat, index) => (
          ...
          >
            ...
            <h3
              className={`text-3xl md:text-4xl font-bold mb-2 ${currentVariant.number}`}
            >
              {stat.number}
            </h3>
